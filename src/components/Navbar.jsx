...
const Navbar = () => {
  ...
  return (
    <nav className="w-full flex py-6 justify-between items-center navbar">
      <img src={logo} alt="hoobank" className="w-[124px] h-[32px] z-50" />

      <ul className="list-none sm:flex hidden justify-end items-center flex-1 z-50">
        {navLinks.map((nav, index) => (
          <li key={nav.link} className={`font-poppins font-normal cursor-pointer text-[16px] 
          ${index === navLinks.length - 1 ? "mr-0" : "mr-10"} text-white`}>
            <Link to={nav.link} className="hover:text-[#08F0E2] ease-in duration-[.3s]">
              {nav.title}
            </Link>
          </li>
        ))}
